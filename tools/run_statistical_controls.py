#!/usr/bin/env python3
"""Run interpretable controls around the global 20x4 compatibility matrix.

A. C-backbone-only-context control
   Hide all designable target-chain tokens while evaluating loss on canonical
   interface positions. This tests dependence of Stage-C on known same-chain
   sequence context outside the target interface.

B. Fixed empirical-PMI control
   Build a 20x4 AA/base PMI from full-heavy-atom contacts in complex-development
   structures only, using the same frozen canonical interface cutoff. The matrix
   is a separate statistical-potential reference and never initializes primary C.
"""
...
def train_c_backbone_only(
    ...
    train = ManifestTable(train_manifest)
    val = ManifestTable(val_manifest)
    max_epochs = development_schedule_horizon(cfg, Stage.GLOBAL_C)
    patience = int(cfg["optimization"]["early_stopping_patience"])
    total_steps = max_epochs * max(1, len(train))
            ...
            torch.save(
                {
                    ...
                    "stage": "global_c_backbone_only",
                    "epoch": epoch,
                    "schedule_horizon_epochs": max_epochs,
                    "schedule_progress": schedule_progress(epoch, max_epochs),
                    "val_metric": score,
                    "config": cfg,
                },
                best_path,
...
def refit_c_backbone_only(
    cfg,
    init_checkpoint,
    selected_dev_checkpoint,
    full_manifest,
    out_dir,
    device=None,
):
    epochs = selected_epoch_count(selected_dev_checkpoint)
    horizon = development_schedule_horizon(cfg, Stage.GLOBAL_C)
    seed = int(cfg["experiment"]["pilot_seed"])
    _seed(seed)
    dev = torch.device(device or ("cuda" if torch.cuda.is_available() else "cpu"))
    model = build_model_from_config(cfg).to(dev)
    ...
    # Replay development schedule prefix rather than compressing it to selected epochs.
    ...
    for epoch in range(epochs):
        ...
        for idx, row in enumerate(rows):
            ...
            optimizer.zero_grad(set_to_none=True)
            with _autocast(cfg, dev):
                loss = _c_control_loss(model, sample, ((idx + epoch) % 2 == 0))
            loss.backward()
            torch.nn.utils.clip_grad_norm_(
                [p for p in model.parameters() if p.requires_grad],
                float(cfg["optimization"]["grad_clip_norm"]),
...
def _pmi_from_dev(
    dev_manifest: Path, cutoff: float, pseudocount: float = 0.5
):
...
def fixed_pmi_checkpoint(
    cfg, prior_checkpoint: Path, dev_manifest: Path, out_dir: Path
):
    model = build_model_from_config(cfg)
    model.load_state_dict(torch.load(prior_checkpoint, map_location="cpu")["model"])
    cutoff = float(cfg["structure_filters"]["interface_contact_angstrom"])
    pmi, counts = _pmi_from_dev(dev_manifest, cutoff)
    with torch.no_grad():
        model.dmicf.global_c.raw.copy_(torch.from_numpy(pmi))
    out_dir.mkdir(parents=True, exist_ok=True)
    np.save(out_dir / "development_empirical_PMI.npy", pmi)
    np.save(out_dir / "development_contact_counts.npy", counts)
    path = out_dir / "fixed_pmi.pt"
    torch.save(
        {
            "model": model.state_dict(),
            "stage": "fixed_empirical_pmi",
            "pmi_source": str(dev_manifest),
...
def evaluate_simple(
    cfg, checkpoint, test_manifest, out_dir, model_name, device=None
):
    dev = torch.device(device or ("cuda" if torch.cuda.is_available() else "cpu"))
    model = build_model_from_config(cfg).to(dev)
    ...
    for row in ManifestTable(test_manifest).rows():
        ...
        s.append(
            partner_scramble(
                model,
                sample,
                int(cfg["evaluation"].get("scramble_repeats", 20)),
                seed,
            )
        )
    core = out_dir / "core"
    ...
    for name, frames in [
        ("conditional_protein", p),
        ("conditional_rna", r),
        ("joint_teacher_forced", j),
        ("partner_scramble", s),
    ]:
        pd.concat(frames, ignore_index=True).to_csv(
            core / f"{name}.tsv", sep="\t", index=False
        )
...
def main():
    ...
    parser.add_argument(
        "--primary-root",
        type=Path,
        default=Path("artifacts/pilot_experiments/training"),
    )
    parser.add_argument(
        "--out", type=Path, default=Path("artifacts/statistical_controls")
    )
    ...
    seeds = args.seeds or [
        int(x) for x in base["experiment"]["primary_training_seeds"]
    ]
    run_rows = []
    for seed in seeds:
        ...
        c_dev = train_c_backbone_only(
            cfg,
            dev_prior,
            args.manifest_root / "complex_train.tsv",
            args.manifest_root / "complex_val.tsv",
            args.out / "training" / "c_backbone_only" / f"seed{seed}" / "development",
            args.device,
        )
        c_refit = refit_c_backbone_only(
            cfg,
            refit_prior,
            c_dev,
            args.manifest_root / "complex_dev.tsv",
            args.out / "training" / "c_backbone_only" / f"seed{seed}" / "refit_full1000",
            args.device,
        )
        c_eval = args.out / "evaluation" / "c_backbone_only" / f"seed{seed}"
        evaluate_simple(
            cfg,
            c_refit,
            args.manifest_root / "complex_test.tsv",
            c_eval,
            "C_backbone_only",
            args.device,
        )
        run_rows.append(
            {"model": "C_backbone_only", "seed": seed, "run_dir": str(c_eval.resolve())}
        )

        pmi_ckpt = fixed_pmi_checkpoint(
            cfg,
            refit_prior,
            args.manifest_root / "complex_dev.tsv",
            args.out / "training" / "fixed_PMI" / f"seed{seed}",
        )
        pmi_eval = args.out / "evaluation" / "fixed_PMI" / f"seed{seed}"
        evaluate_simple(
            cfg,
            pmi_ckpt,
            args.manifest_root / "complex_test.tsv",
        ...
        run_rows.append(
            {
                "model": "fixed_empirical_PMI",
                "seed": seed,
                "run_dir": str(pmi_eval.resolve()),
            }
        )
    args.out.mkdir(parents=True, exist_ok=True)
    pd.DataFrame(run_rows).to_csv(
        args.out / "statistical_control_runs.tsv", sep="\t", index=False
    )
