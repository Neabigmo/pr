...
``partner_blind`` forces cross-molecular corrections to zero.
``geometry_only`` preserves cross-structure capacity and geometry but averages
compatibility across partner identity, preventing specific AA/base coupling.

Both controls share the primary pretrained priors, frozen manifests, stage order,
development selection and full-1,000 refit schedule semantics.
"""
...
from pr_pilot.training.engine import _adapter, _autocast, _cosine_schedule, _one_training_loss, build_model_from_config, validate_stage
from pr_pilot.training.refit import schedule_horizon_epochs, schedule_progress, selected_epoch_count
...
def install_control_mode(model: JointPriorAndFieldModel, mode: str) -> None:
    ...
    def controlled_forward(self, *args, **kwargs):
        ...
        if mode == "partner_blind":
            ...
            return out
        if len(args) < 7:
            raise ValueError("Controlled forward requires standard positional JointPriorAndFieldModel call")
        pr = args[6]
...
def train_control_stage(cfg: dict, mode: str, stage: Stage, train_manifest: Path, val_manifest: Path, init_checkpoint: Path, out_dir: Path, device: str | None = None) -> Path:
    ...
    for epoch in range(max_epochs):
        ...
        if stage == Stage.JOINT: apply_joint_unfreezing(model, progress)
        adapter = _adapter(cfg, epoch, training=True)
        rows = list(train_table.rows()); random.Random(seed + epoch * 104729).shuffle(rows)
        losses = []
        for row in rows:
            optimizer.zero_grad(set_to_none=True)
            with _autocast(cfg, dev): loss, _ = _one_training_loss(model, row, adapter, stage, cfg, epoch, progress, dev)
            if not torch.isfinite(loss): raise FloatingPointError(f"Non-finite {mode} loss at {stage.value} {row.sample_id}")
            loss.backward()
        ...
        with metrics_path.open("a", encoding="utf-8") as handle:
            handle.write(json.dumps({"control_mode": mode, "stage": stage.value, "epoch": epoch, "train_loss": float(np.mean(losses)), "val_metric": val, "trainable": trainable_parameter_report(model)}, sort_keys=True) + "\n")
        if val < best - 1e-6:
            best = val; bad_epochs = 0
            torch.save({"model": model.state_dict(), "stage": stage.value, "control_mode": mode, "epoch": epoch, "val_metric": val, "config": cfg}, best_path)
        else:
            bad_epochs += 1
            if bad_epochs >= patience: break
    if not best_path.exists(): raise RuntimeError("Control stage saved no checkpoint")
    return best_path


def refit_control_stage(cfg: dict, mode: str, stage: Stage, full_manifest: Path, selected_dev_checkpoint: Path, init_checkpoint: Path, out_dir: Path, device: str | None = None) -> Path:
    """Validation-free refit that replays the development schedule prefix."""
    epochs = selected_epoch_count(selected_dev_checkpoint)
    horizon = schedule_horizon_epochs(cfg, stage)
    if epochs > horizon:
        raise ValueError(f"Control selected epochs {epochs} exceed schedule horizon {horizon}")
    seed = int(cfg["experiment"]["pilot_seed"])
    random.seed(seed); np.random.seed(seed); torch.manual_seed(seed)
    if torch.cuda.is_available(): torch.cuda.manual_seed_all(seed)
    dev = torch.device(device or ("cuda" if torch.cuda.is_available() else "cpu"))
    ...
    total_schedule_steps = horizon * max(1, len(table)); global_step = 0
    out_dir.mkdir(parents=True, exist_ok=True); metrics = out_dir / "refit_metrics.jsonl"

    for epoch in range(epochs):
        model.train(); progress = schedule_progress(epoch, horizon)
        if stage == Stage.JOINT: apply_joint_unfreezing(model, progress)
        adapter = _adapter(cfg, epoch, training=True)
        rows = list(table.rows()); random.Random(seed + epoch * 104729).shuffle(rows)
        losses = []
        for row in rows:
            optimizer.zero_grad(set_to_none=True)
            with _autocast(cfg, dev): loss, _ = _one_training_loss(model, row, adapter, stage, cfg, epoch, progress, dev)
            if not torch.isfinite(loss): raise FloatingPointError(f"Non-finite {mode} refit loss at {stage.value} {row.sample_id}")
            loss.backward()
            torch.nn.utils.clip_grad_norm_([p for p in model.parameters() if p.requires_grad and p.grad is not None], float(cfg["optimization"]["grad_clip_norm"]))
            optimizer.step(); global_step += 1
        ...
        with metrics.open("a", encoding="utf-8") as handle:
            handle.write(json.dumps({"control_mode": mode, "stage": stage.value, "epoch": epoch, "selected_epoch_count": epochs, "schedule_horizon_epochs": horizon, "schedule_progress": progress, "train_loss": float(np.mean(losses)), "validation_used": False}, sort_keys=True) + "\n")

    checkpoint = out_dir / "refit.pt"
    torch.save({"model": model.state_dict(), "stage": stage.value, "control_mode": mode, "epoch": epochs - 1, "refit": True, "validation_used": False, "selected_epoch_count": epochs, "schedule_horizon_epochs": horizon, "schedule_progress_at_stop": schedule_progress(epochs - 1, horizon), "config": cfg}, checkpoint)
    return checkpoint


def run_control_pipeline(cfg: dict, mode: str, manifests: Path, primary_development_prior: Path, primary_refit_prior: Path, out_dir: Path, device: str | None = None) -> Path:
    dev_prev = primary_development_prior; refit_prev = primary_refit_prior
    for stage in CONTROL_STAGES:
        dev_stage = out_dir / "development" / stage.value
        dev_checkpoint = train_control_stage(cfg, mode, stage, manifests / "complex_train.tsv", manifests / "complex_val.tsv", dev_prev, dev_stage, device)
        refit_stage_dir = out_dir / "refit_full1000" / stage.value
        refit_checkpoint = refit_control_stage(cfg, mode, stage, manifests / "complex_dev.tsv", dev_checkpoint, refit_prev, refit_stage_dir, device)
        dev_prev = dev_checkpoint; refit_prev = refit_checkpoint
