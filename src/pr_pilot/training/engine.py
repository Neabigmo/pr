...
def _drop_intra_edges(graph: PolymerGraph, probability: float, seed: int) -> None:
    """Drop non-covalent spatial edges while preserving graph connectivity basics."""
    p = float(probability)
...
def _drop_pr_edges(sample: ComplexTensorSample, probability: float, seed: int) -> None:
    ...
    for labels in (pi, ri):
        for node in np.unique(labels):
            ...
            if len(idx):
                nearest = idx[np.argmin(dist[idx])]
                keep[nearest] = True
...
def _one_training_loss(
    ...
    if stage in {Stage.GLOBAL_C, Stage.DELTA_C, Stage.ALPHA}:
        ...
        if protein_target:
            ...
            rknown = sample.rna.valid.clone().bool()
            rmask = torch.zeros_like(sample.rna.valid)
            task = "protein"
            target_graph = sample.protein
            target_tokens, target_known, target_mask = ptok, pknown, pmask
        else:
            rtok, rknown, rmask = _all_interface_corruption(sample.rna)
            ptok = sample.protein.sequence.clone()
            pknown = sample.protein.valid.clone().bool()
        ...
        if stage == Stage.ALPHA:
            if task == "protein":
                ...
                pknown = hide_known_partner_tokens(
                    pknown,
                    sample.protein.fixed,
                    row.sample_id,
                    epoch,
                    seed,
                    float(mcfg["partner_token_dropout"]),
    ...
    schedule = TaskRatioSchedule(
        ...
        tuple(cfg["training_stages"]["joint"]["task_ratio_end"]),
    )
    ...
    if task_name == "protein_conditional":
        ...
        ptok = sample.protein.sequence.clone()
        pknown = hide_known_partner_tokens(
            sample.protein.valid.clone().bool(),
            sample.protein.fixed,
            row.sample_id,
            epoch,
            seed,
    ...
    for row in manifest.rows():
        with _autocast(cfg, device):
            if stage == Stage.PROTEIN_PRIOR:
                ...
                values.append(float(breakdown.total))
                continue
            if stage == Stage.RNA_PRIOR:
                graph = _move_graph(load_rna_row(adapter, row), device)
                known = graph.fixed & graph.valid
...
def train_stage(
    ...
    """Train one stage and return the validation-selected checkpoint."""
    seed = int(cfg["experiment"]["pilot_seed"])
    random.seed(seed)
    np.random.seed(seed)
    ...
    optimizer = build_optimizer(
        ...
        float(optcfg["lr_global_c_joint"]),
