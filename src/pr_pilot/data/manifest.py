...
2. Protein P30 and RNA Rfam/R80 overlap is checked at the constituent-chain level.
3. Final-test homologues/families are purged from Protein/RNA structural-prior pools.
4. The 900/100 single-molecule development splits are themselves group-disjoint:
   Protein by exact sequence/P30; RNA by exact sequence/R80/Rfam.
5. All randomisation is stable hash-based and independent of dataframe row order.
"""
...
def bilateral_components(df: pd.DataFrame) -> list[list[str]]:
    """Connected complex components under any shared P30, Rfam or R80 label."""
...
def single_molecule_components(df: pd.DataFrame, molecule: str) -> list[list[str]]:
    """Components for leakage-safe single-molecule train/validation selection.

    Exact sequence is always a grouping edge. Protein additionally groups by any
    P30 label. RNA additionally groups by any R80 or Rfam label. This prevents
    early-stopping validation from being an easier near-duplicate task than the
    intended structural-prior generalization problem.
    """
    required = REQUIRED_PROTEIN if molecule == "protein" else REQUIRED_RNA
    _require_columns(df, required, molecule)
    frame = df.reset_index(drop=True).copy()
    if frame["sample_id"].astype(str).duplicated().any():
        raise ValueError(f"Duplicate sample_id in eligible {molecule} candidates")
    uf = _UnionFind(len(frame))
    last: dict[str, int] = {}
    for i, row in frame.iterrows():
        labels = {f"hash:{row['sequence_hash']}"}
        if molecule == "protein":
            p30 = _labels(row["protein_cluster_p30"])
            if not p30:
                raise ValueError(f"Protein {row['sample_id']} has no usable P30 label")
            labels.update(f"p30:{x}" for x in p30)
        elif molecule == "rna":
            rlabels = _rna_holdout_labels(row)
            if not rlabels:
                raise ValueError(f"RNA {row['sample_id']} has neither usable R80 nor Rfam label")
            labels.update(rlabels)
        else:
            raise ValueError("molecule must be 'protein' or 'rna'")
        for label in labels:
            if label in last:
                uf.union(i, last[label])
            else:
                last[label] = i
    groups: dict[int, list[str]] = {}
    for i, sid in enumerate(frame["sample_id"].astype(str)):
...
def single_molecule_group_split(df: pd.DataFrame, molecule: str, n_train: int, n_val: int, seed: int) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Choose an exact-size group-disjoint 900/100 split from an oversized pool."""
    if len(df) < n_train + n_val:
        raise ValueError(f"Need at least {n_train+n_val} eligible {molecule} rows after final-test purge; found {len(df)}")
    val_ids = _exact_component_subset(single_molecule_components(df, molecule), n_val, seed + 1)
    val = df[df["sample_id"].astype(str).isin(val_ids)].copy().reset_index(drop=True)
    remaining = df[~df["sample_id"].astype(str).isin(val_ids)].copy().reset_index(drop=True)
    train = deterministic_sample(remaining, n_train, seed + 2)
    assert_single_molecule_split_disjoint(train, val, molecule)
    return train, val
...
def assert_single_molecule_split_disjoint(train: pd.DataFrame, val: pd.DataFrame, molecule: str) -> None:
    exact = set(train["sequence_hash"].astype(str)) & set(val["sequence_hash"].astype(str))
    if exact:
        raise AssertionError(f"{molecule} exact-sequence leakage across train/val: {len(exact)}")
    if molecule == "protein":
        overlap = _flatten_column(train, "protein_cluster_p30") & _flatten_column(val, "protein_cluster_p30")
        if overlap:
            raise AssertionError(f"Protein P30 leakage across train/val: {sorted(overlap)[:5]}")
    elif molecule == "rna":
        r80 = _flatten_column(train, "rna_cluster_r80") & _flatten_column(val, "rna_cluster_r80")
        rfam = _flatten_column(train, "rfam_family") & _flatten_column(val, "rfam_family")
        if r80 or rfam:
            raise AssertionError(f"RNA train/val leakage: R80={sorted(r80)[:5]} Rfam={sorted(rfam)[:5]}")
    else:
...
def freeze_single_molecule_pool(
    ...
    """Freeze exactly 1,000 prior structures with strict 900/100 development split."""
    ...
    if train_n + val_n != pool_n:
        raise ValueError(f"{molecule} train/val counts must exactly equal frozen pool size")
    train, val = single_molecule_group_split(df, molecule, train_n, val_n, seed)
    pool = pd.concat([train, val], ignore_index=True)
    if len(pool) != pool_n:
        raise AssertionError(f"Frozen {molecule} pool size mismatch")
    ...
    _write_manifest_meta(
        ...
        extra={
            ...
            "train_validation_grouping": "exact+P30" if molecule == "protein" else "exact+R80+Rfam",
        },
    )
    return paths
...
def freeze_complex_pool(
    ...
    for label, frame in frames.items():
        ...
        frame.to_csv(path, sep="\t", index=False)
        paths[label] = path
    _write_manifest_meta(out_dir / "complex_manifest_meta.json", seed, paths, extra={"relaxations": relaxations, "strict_validation": strict_validation})
    return paths
...
def assert_no_test_leakage(train: pd.DataFrame, val: pd.DataFrame, test: pd.DataFrame, strict_cluster_check: bool = True) -> None:
    ...
    if strict_cluster_check and len(dev):
        ...
        r_overlap = _flatten_column(dev, "rfam_family") & _flatten_column(test, "rfam_family")
        if r_overlap:
...
def _write_manifest_meta(path: Path, seed: int, paths: dict[str, Path], extra: dict | None = None) -> None:
    payload = {
        "seed": seed,
        "files": {name: {"path": str(file_path), "sha256": sha256_file(file_path)} for name, file_path in paths.items()},
    }
    if extra:
