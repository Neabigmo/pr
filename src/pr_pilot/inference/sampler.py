...
def _forward(model: JointPriorAndFieldModel, sample: ComplexTensorSample, pt: Tensor, rt: Tensor, pk: Tensor, rk: Tensor) -> dict[str, Tensor]:
    return model(
...
def _reopen_uncertain(model: JointPriorAndFieldModel, sample: ComplexTensorSample, pt: Tensor, rt: Tensor, polymer: str, fraction: float) -> list[int]:
...
def _refine_polymer(model: JointPriorAndFieldModel, sample: ComplexTensorSample, pt: Tensor, rt: Tensor, polymer: str, reopen: list[int], temperature: float, generator: torch.Generator, rng: random.Random) -> None:
...
def sample_joint(
    ...
    for candidate_index in range(candidates):
        ...
        pt[~pk] = 0; rt[~rk] = 0
        order = _design_positions(sample, order_mode, rng); logprobs = []
        ...
        if spir_enabled and spir_cycles > 0:
            ...
            for _ in range(spir_cycles):
                if direction == "P_then_R":
                    ...
                    reopen_p = _reopen_uncertain(model, sample, pt, rt, "P", spir_reopen_fraction)
                    _refine_polymer(model, sample, pt, rt, "P", reopen_p, spir_temperature, generator, rng)
        results.append(Candidate(f"{sample.sample_id}__{order_mode}__{candidate_index:04d}", pt.clone(), rt.clone(), pre_p, pre_r, logprobs, spir_cycles if spir_enabled else 0, direction, order_mode))
    return results
